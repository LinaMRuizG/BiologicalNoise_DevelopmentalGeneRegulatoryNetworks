...
(* ::Text:: *)
(*This scripts is almost the same than unRegulatedBurst . m but here we only run the simulation for a set of parameters that were chosen randomly for grids*)
...
(*ini = steadyStateStart[window,listData];*)
        {mRNAffcv2ItA, proteinffcv2ItA} = Table[{ff[i], cv[i],cv2[i], Mean[i],StandardDeviation[i],Variance[i],Kurtosis[i],Skewness[i],Table[Moment[i,j],{j,10}],Table[CentralMoment[i,j],{j,10}]}, {i, {listData[[ini ;; All, 1]], listData[[ini ;; All, 2]]}}];
...
(*AppendTo[listEstimatesPara,listEstimatesReplicates];*)
     Export["controlUnRGA_sample_"<>ToString[sample]<>","<>ToString[para]<>".csv",listEstimatesReplicates];
    ,
 {para,paraV}];
