...
(* ::Text:: *)
(*In this nb is simulated a One UnR gene A with  GA and constitutive expression. The idea is to evaluate 4 regions of parameters  *)
...
kpA=1.386;(* protein production rate (molecules*t^-1) *)
\[Gamma]pA=0.02082;(* protein degradation rate (molecules*t^-1) *)
...
(* ::Input::Initialization:: *)
changesByRXN=Function/@<|1->{#1+1,#2},2->{#1-1,#2},3->{#1,#2+1},4->{#1,#2-1}|>;(* Each hash represents a specie: mRNA,protein, da0:promoterInactivated,da1: promoterActivated. The numbers are the next reactions: 1-activation, 2-deactivation, 3-5 synthesis, 4-6 degradation *)pF=Compile[{{km,_Real},{\[Gamma]m,_Real},{kp,_Real},{\[Gamma]p,_Real},{a,_Integer},{p,_Integer}},{km+0.01,a*\[Gamma]m,kp*a,\[Gamma]p*p},RuntimeAttributes->{Listable},Parallelization->True];
...
(* ::Input::Initialization:: *)
simu[iteraciones_,replicas_,kmA_,\[Gamma]mA_,kpA_,\[Gamma]pA_,rep_,parametros_,lagStep_,species_]:=
...
DistributeDefinitions[changesByRXN,pF,ff,expressionNoise,iteraciones,kmA,\[Gamma]mA,kpA,\[Gamma]pA];
...
Do[
          \[Tau]is=Map[If[#>0,RandomVariate[ExponentialDistribution[#]],\[Infinity]]&,pF[kmA,\[Gamma]mA,kpA,\[Gamma]pA,Sequence@@abundancia]];reactionAnd\[Tau]\[Mu]={FirstPosition[#,Min[#]],Min[#]}&[\[Tau]is];abundancia=Through[{reactionAnd\[Tau]\[Mu][[1]]/.changesByRXN}[[1,1]]@@abundancia][[1]];t=t+reactionAnd\[Tau]\[Mu][[2]];listDataAndTime["Union",{{t,abundancia}}];
...
Export["controlUnRGAConst_"<>parametros<>".csv",{listEstimatemRNAA,listEstimateproteinA}];
...
(* ::Input::Initialization:: *)
s=simu[iteraciones2,replicas2,kmA,\[Gamma]mA,kpA,\[Gamma]pA,rep,"\n Default parameters"(*parametros*),lagStep,species];


(* ::Input::Initialization:: *)
Export["controlUnRGAConstReg.pdf",s];
