...
changesByRXN=Function/@<|1->{#1,#2,#3,#4,#5-1,#6+1,#7,#8},2->{#1,#2,#3,#4,#5+1,#6-1,#7,#8},3->{#1+1,#2,#3,#4,#5,#6,#7,#8},4->{#1,#2+1,#3,#4,#5,#6,#7,#8},5->{#1-1,#2,#3,#4,#5,#6,#7,#8},6->{#1,#2-1,#3,#4,#5,#6,#7,#8},7->{#1,#2,#3+1,#4,#5,#6,#7,#8},8->{#1,#2,#3,#4+1,#5,#6,#7,#8},9->{#1,#2,#3-1,#4,#5,#6,#7,#8},10->{#1,#2,#3,#4-1,#5,#6,#7,#8}|>;(* Each hash represents a specie: mRNAA-proteinA-mRNAB-proteinB-da0A:promoterActivatedA-da1A:promoterInactivatedA-db0B-db1B
 ...
 1\[Rule] Activation,
 2\[Rule] Desactivation,
 3-4-7-8\[Rule] Synthesis,
5-6-9-10\[Rule] Degradation*)
pF=Compile[{{h,_Real},{kAB,_Real},{kgA,_Real},{\[Gamma]gA,_Real},{kmA,_Real},{\[Gamma]mA,_Real},{kpA,_Real},{\[Gamma]pA,_Real},{kgB,_Real},{\[Gamma]gB,_Real},{kmB,_Real},{\[Gamma]mB,_Real},{kpB,_Real},{\[Gamma]pB,_Real},{mRNAA,_Integer},{proteinA,_Integer},{mRNAB,_Integer},{proteinB,_Integer},{d0A,_Integer},{d1A,_Integer},{d0B,_Integer},{d1B,_Integer}},{d0A*kgA,d1A*\[Gamma]gA,d1A*kmA+0.01,kpA*mRNAA,mRNAA*\[Gamma]mA,\[Gamma]pA*proteinA,kmB*(1/(1+(proteinA/kAB)^h))+0.01,kpB*mRNAB,mRNAB*\[Gamma]mB,\[Gamma]pB*proteinB},RuntimeAttributes->{Listable},Parallelization->True];
...
expressionNoise[data_]:=N[Variance[data]/Mean[data]^2](* Este sale del art\[IAcute]culo Franz.2011*)
