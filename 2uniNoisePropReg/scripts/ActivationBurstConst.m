...
(* ::Input::Initialization:: *)
changesByRXN=Function/@<|1->{#1,#2,#3,#4,#5-1,#6+1,#7,#8},2->{#1,#2,#3,#4,#5+1,#6-1,#7,#8},3->{#1+1,#2,#3,#4,#5,#6,#7,#8},4->{#1,#2+1,#3,#4,#5,#6,#7,#8},5->{#1-1,#2,#3,#4,#5,#6,#7,#8},6->{#1,#2-1,#3,#4,#5,#6,#7,#8},7->{#1,#2,#3+1,#4,#5,#6,#7,#8},8->{#1,#2,#3,#4+1,#5,#6,#7,#8},9->{#1,#2,#3-1,#4,#5,#6,#7,#8},10->{#1,#2,#3,#4-1,#5,#6,#7,#8}|>;(* Each hash represents a specie: mRNAA-proteinA-mRNAB-proteinB-da0A:promoterActivatedA-da1A:promoterInactivatedA-db0B-db1B
   The numbers are the next reactions: 1\[Rule] Activation, 2\[Rule] Desactivation, 3-4-7-8\[Rule] Synthesis, 5-6-9-10\[Rule] Degradation*)
pF=Compile[{{h,_Real},{kAB,_Real},{kgA,_Real},{\[Gamma]gA,_Real},{kmA,_Real},{\[Gamma]mA,_Real},{kpA,_Real},{\[Gamma]pA,_Real},{kgB,_Real},{\[Gamma]gB,_Real},{kmB,_Real},{\[Gamma]mB,_Real},{kpB,_Real},{\[Gamma]pB,_Real},{mRNAA,_Integer},{proteinA,_Integer},{mRNAB,_Integer},{proteinB,_Integer},{d0A,_Integer},{d1A,_Integer},{d0B,_Integer},{d1B,_Integer}},{d0A*kgA,d1A*\[Gamma]gA,d1A*kmA+0.01,kpA*mRNAA,mRNAA*\[Gamma]mA,\[Gamma]pA*proteinA,kmB*(proteinA^h/(kAB+proteinA ^h))+0.01,kpB*mRNAB,mRNAB*\[Gamma]mB,\[Gamma]pB*proteinB},RuntimeAttributes->{Listable},Parallelization->True];
...
Export["controlABGAActivRegBurstConst_"<>parametros<>".csv",{listEstimatemRNAA,listEstimateproteinA,listEstimatemRNAI,listEstimateproteinI}];
...
Export["controlABGAActivRegBurstConst.pdf",s];


(* ::Input::Initialization:: *)
(* Use this code when you will simulate with different parameters values for gene A: Activator-Regulator *)
sA=Flatten[Table[simu[iteraciones2,replicas2,nRep,h,kAB,kgA,\[Gamma]gA,kmA,\[Gamma]mA,kpA,\[Gamma]pA,kgB,\[Gamma]gB,kmB,\[Gamma]mB,kpB,\[Gamma]pB,rep,"kgA" <>ToString[kgA]<>"\[Gamma]gA"<>ToString[\[Gamma]gA](*parametros*),lagStep,species],{kgA,{0.05,1}},{\[Gamma]gA,{0.2,2}}],{3}];
Export["controlABGAActivRegBurstConst.pdf",sA];


(* ::Input::Initialization:: *)
(* Use this code when you will simulate with different parameters values for gene B: Activated-Regulated *)
sB=Flatten[Table[simu[iteraciones2,replicas2,nRep,h,kAB,kgA,\[Gamma]gA,kmA,\[Gamma]mA,kpA,\[Gamma]pA,kgB,\[Gamma]gB,kmB,\[Gamma]mB,kpB,\[Gamma]pB,rep,"kgB" <>ToString[kgB]<>"\[Gamma]gB"<>ToString[\[Gamma]gB](*parametros*),lagStep,species],{kgB,{0.05,1}},{\[Gamma]gB,{0.2,2}}],{3}];
Export["controlABGAActiRegkdp2.pdf",sB];
